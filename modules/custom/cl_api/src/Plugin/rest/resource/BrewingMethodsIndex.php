 ...
 *
 * @RestResource(
 *   id = "brewing_methods_index",
 *   label = @Translation("Brewing methods index"),
 *   uri_paths = {
 *     "canonical" = "/api/brewing-method"
 *   }
 * )
 */
class BrewingMethodsIndex extends ResourceBase {
  ...
  public function get() {
    ...
    if (!$this->currentUser->hasPermission('access brewing methods resource')) {
      ...
      $query->condition('vid', 'brewing_methods');
      $ids = $query->execute();
      $taxonomy_terms = $storage->loadMultiple($ids);
      $data = [];
      if ($taxonomy_terms) {
        foreach ($taxonomy_terms as $taxonomy_term) {
          $data[$taxonomy_term->id()] = BrewingMethodsHelper::formatBrewingMethod($taxonomy_term);
        }
      }
